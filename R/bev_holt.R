#' Basic Beverton-Holt model
#' @param path (character) path to write .jags file
#' @export
write_jags_model.bev_holt <- function(path){
  mod <-
    "model{
    ...
    for (r in 1:2){
      ...
      R_msy[r] <- (alpha_prime[r]*S_msy[r])/(1+beta[r]*S_msy[r])
      MSY[r] <- R_msy[r]-S_msy[r]
      S_max[r] <- 1/beta[r]
      MSR[r] <- (alpha_prime[r]*S_max[r])/(1+beta[r]*S_max[r])
    ...
    for (i in 1:450){
      S_star[i] <- 50*i
      for (r in 1:2){
        R_star[i,r] <- (alpha_prime[r]*S_star[i])/(1+beta[r]*S_star[i])

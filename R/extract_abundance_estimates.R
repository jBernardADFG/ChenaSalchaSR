...
extract_abundance_estimates <- function(samples, alpha=0.10, file_path){
  ...
  chena_abundance <- data.frame(year=1986:2020, total_run = rep(NA, 35), inriver_run = rep(NA, 35), escapement = rep(NA, 35))
  salcha_abundance <-  data.frame(year=1986:2020, total_run = rep(NA, 35), inriver_run = rep(NA, 35), escapement = rep(NA, 35))
  
  chena_abundance[9:35, 2] <- paste(ct_quants[1,], " (", ct_quants[2,], ", " ,  ct_quants[3,], ")", sep="")
  salcha_abundance[9:35, 2] <- paste(st_quants[1,], " (", st_quants[2,], ", " ,  st_quants[3,], ")", sep="")
  chena_abundance[,3] <- paste(ci_quants[1,], " (", ci_quants[2,], ", " ,  ci_quants[3,], ")", sep="")
  salcha_abundance[,3] <- paste(si_quants[1,], " (", si_quants[2,], ", " ,  si_quants[3,], ")", sep="") 
  chena_abundance[,4] <- paste(ce_quants[1,], " (", ce_quants[2,], ", " ,  ce_quants[3,], ")", sep="")
  salcha_abundance[,4] <- paste(se_quants[1,], " (", se_quants[2,], ", " ,  se_quants[3,], ")", sep="")

...
age_hist <- function(samples, file_path, width=500, height=1000){
  ...
  med_array <- array(NA, dim=c(27, 2, 6))
  l <- 1
  for(k in 1:6){
    for(j in 1:2){
      for(i in 1:27){
  ...
  chena_n <- data.frame(year=rep(1994:2020, rep(6, length(1994:2020))),
                          Age=rep(1:6, length(1994:2020)),
                          run=rep(NA, 6*length(1994:2020)))
  salcha_n <- data.frame(year=rep(1994:2020, rep(6, length(1994:2020))),
                          Age=rep(1:6, length(1994:2020)),
                          run=rep(NA, 6*length(1994:2020)))
  k <- 1
  ...
  med_array <-  array(NA, dim=c(35, 2, 6))
  l <- 1
  for(k in 1:6){
    for(j in 1:2){
      for(i in 1:35){
  ...
  chena_p <- data.frame(year=rep(1994:2020, rep(6, length(1994:2020))),
                        Age=rep(1:6, length(1994:2020)),
                        run=rep(NA, 6*length(1994:2020)))
  salcha_p <- data.frame(year=rep(1994:2020, rep(6, length(1994:2020))),
                         Age=rep(1:6, length(1994:2020)),
                         run=rep(NA, 6*length(1994:2020)))
  k <- 1
  for(i in 1:nrow(chena_tab)){
  ...
  p4 <- ggplot2::ggplot(salcha_p, aes(x=year, y=run, fill=Age))+geom_col()+theme_classic()+
    ...
    scale_x_continuous(breaks=1994:2020)
  print(ggpubr::ggarrange(p1, p2, p3, p4,
